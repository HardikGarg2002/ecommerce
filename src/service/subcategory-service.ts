import { ISubcategory, ISubcategoryWithMeta } from '../interface/subcategory';
...
export default class SubcatService {
	...
	public async getById(id: string): Promise<ISubcategory> {
		...
		if (!subcategory) {
			throw APIError.BusinessError('subcategory not found', 'ERR_NOT_FOUND');
		}
	...
	public async getByCode(code: string, statusError: boolean = false): Promise<ISubcategory> {
		...
		if (statusError === true && subcategory.is_active === false) {
			throw APIError.BusinessError('subcategory Inactive', 'ERR_INACTIVE_SUBCATEGORY');
		}
		return subcategory;
	}
	...
	public async patch(
		...
		// 	throw APIError.BusinessError(`Inactive subcategory can not be modified`, 'ERR_SUBCATEGORY_INACTIVE');
		// }
		...
		await this.save(updatedSubcategory);
	}
	...
	private async _validateNameAndCodeUnique(name: string, code?: string) {
		...
		if (matchedCat.length > 0) {
			throw APIError.BusinessError(
				'subcategory with the same name and code already exists in database',
				'ERR_DUPLICATE',
			);
		}
	}
