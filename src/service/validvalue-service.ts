...
import { IValidvalue, IValidvalueWithMeta, IValue } from '../interface/validvalue';
...
export default class ValidvalueService {
	...
	public async getByType(type: string, fetch: string = 'active'): Promise<IValidvalue> {
		...
		if (!validvalue) {
			throw APIError.BusinessError(
				`valid value with type ${type.toUpperCase()} not exists in database`,
				'ERR_NOT_FOUND',
			);
		}
	...
	public async getById(id: string, fetch: string = 'active'): Promise<IValidvalue> {
		...
		if (!validvalue) {
			throw APIError.BusinessError('validvalue not found', 'ERR_NOT_FOUND');
		}
	...
	public async getValue(type: string, key: string): Promise<IValue> {
		...
		if (!value) {
			throw APIError.BusinessError(
				`value with key ${key.toUpperCase()} not  exists in the valid value`,
				'ERR_NOT_FOUND',
			);
		}
		return value;
	...
	public async addValues(type: string, inputValues: IValue[], reason: string, user: IUser) {
		...
		const updatedValidValue: IValidvalue = clone(existingValidvalue);
	...
	public async patchValue(
		...
		if (
			...
			throw APIError.BusinessError('duplicate labels', 'ERR_DUPLICATE_LABELS');
		}
		...
		// 	throw APIError.BusinessError(`Inactive value can not be modified`, 'ERR_VALUE_INACTIVE');
		// }
		...
		const updatedValidValue = clone(existingValidvalue);
	...
	private async _validateLabelOrTypeUnique(label: string, type?: string) {
		...
		if (matchedValue.length > 0) {
			throw APIError.BusinessError(`validvalue with this type/label already exists in database`, 'ERR_DUPLICATE');
		}
	}
	...
	private _addValuesToValidValue = (validvalue: IValidvalue, newValues: IValue[]) => {
		for (let i = 0; i < newValues.length; i++) {
			...
			if (isDuplicate) {
				throw APIError.BusinessError('duplicate values', 'ERR_DUPLICATE_VALUES');
			}
