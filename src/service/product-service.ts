...
import { IProduct, IProductFeatures, IProductImage, IProductPrices, IProductWithMeta } from '../interface/product';
...
import { clone } from '../utils/common-utils';
...
export default class ProductService {
	...
	private _validateCategoryAndSubCategory = async (category_code: string, subcategory_code: string) => {
		...
		if (subcategory && subcategory.category_code !== category_code.toUpperCase())
			throw APIError.BusinessError('Subcategory do not belong to this category', 'INVALID_SUBCATEGORY_CODE');
		// if (subcategory.is_active === false) throw APIError.BusinessError('Subcategory is not active', 'INACTIVE_SUBCATEGORY');
	};
	...
	public async bulkUpdateProducts(filter: any, update: any, updatedBy: IUser): Promise<void> {
		try {
			const modifiedBy = clone(updatedBy);
			...
			throw APIError.internal('Error during update', error);
		}
	}
	...
	private _addFeaturesToProduct = (product: IProduct, newFeatures: IProductFeatures[]) => {
		for (let i = 0; i < newFeatures.length; i++) {
			...
			if (isDuplicate) {
				throw APIError.BusinessError('duplicate features', 'ERR_DUPLICATE_FEATURES');
			}
	...
	public async getById(id: string): Promise<IProduct> {
		...
		if (!product) {
			throw APIError.BusinessError('Product not found', 'ERR_NOT_FOUND');
		}
	...
	public async removeImage(id: string, imageUrl: string, user: IUser): Promise<void> {
		...
		if (product.images.primary === imageUrl.trim()) {
			throw APIError.BusinessError('Primary image can not be deleted', 'ERR_NOT_ALLOWED');
		}
		...
		if (index !== -1) {
			...
			throw APIError.BusinessError('Image not found', 'ERR_NOT_FOUND');
		}
		const updatedProduct = await this.save(product);
	}
	...
	public async removeAlias(id: string, updated: IUser, aliasId: string, reason: string): Promise<void> {
		...
		if (aliasToRemoveIndex === -1) {
			throw APIError.BusinessError(`Alias with id ${aliasId} does not exist in this product.`, 'ERR_ALIAS_NOT_EXISTS');
		}
	...
	public async addFeatures(id: string, user: IUser, inputFeatures: IProductFeatures[]): Promise<void> {
		const existingProduct = await this.getById(id);
		const updatedProduct: IProduct = clone(existingProduct);
	...
	public async deactivateByCategoryCode(code: string, updated: IUser) {
		const modifiedBy = clone(updated);
		modifiedBy.date = new Date();
