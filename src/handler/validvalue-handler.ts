...
export const get = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const validvalues = await validvalueController.get();
...
export const getByType = async (
  ...
  try {
    ...
    const validvalue = await validvalueController.getByType(
      validvalueType,
      fetch
    );
...
export const addValues = async (
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "values added to valid value successfully" });
  } catch (err) {
    next(err);
  }
};
...
export const activateValue = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    console.log(reason);
    await validvalueController.activateValue(
      type,
      key,
      reason,
      user,
    ...
    res
      .status(200)
      .json({ message: `status of the value updated successfully` });
  } catch (err) {
    next(err);
  }
};
